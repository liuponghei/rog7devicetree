...
log -t WifiSARPower enter Wifion=$Wifion Country=$Country ReceiverOn=$ReceiverOn Softapon=$Softapon Slm=$Slm WWANon=$WWANon Camera=$Camera
...
if [ "$Country" == "US" ]; then
    if [ "$Camera" == "0" ]; then
        ...
        if [ "$WWANon" == "1" ] && [ "$Softapon" == "ok" ]; then
            ...
            log -t WifiSARPower US/Camera mode WWANon on and Softapon on
        elif [ "$WWANon" == "1" ]; then
            ...
            log -t WifiSARPower US/Camera mode WWANon on
        elif [ "$WWANon" == "0" ]; then
            ...
            log -t WifiSARPower US/Camera mode
        else
            vendor_cmd_tool -f /vendor/bin/sar-vendor-cmd.xml -i wlan0 --START_CMD --SAR_SET --ENABLE 7 --NUM_SPECS 2 --SAR_SPEC --NESTED_AUTO --CHAIN 0 --POW_IDX 6 --END_ATTR --NESTED_AUTO --CHAIN 1 --POW_IDX 6 --END_ATTR --END_ATTR --END_CMD
